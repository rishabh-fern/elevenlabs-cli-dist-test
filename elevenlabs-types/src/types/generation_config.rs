...
pub struct GenerationConfig {
    ...
    pub chunk_length_schedule: Option<Vec<f64>>,
}

impl GenerationConfig {
    pub fn builder() -> GenerationConfigBuilder {
        <GenerationConfigBuilder as Default>::default()
    }
}
...
pub struct GenerationConfigBuilder {
    chunk_length_schedule: Option<Vec<f64>>,
}

impl GenerationConfigBuilder {
    pub fn chunk_length_schedule(mut self, value: Vec<f64>) -> Self {
        self.chunk_length_schedule = Some(value);
        self
    }

    /// Consumes the builder and constructs a [`GenerationConfig`].
    pub fn build(self) -> Result<GenerationConfig, BuildError> {
        Ok(GenerationConfig {
